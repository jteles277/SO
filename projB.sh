...
SwapArray() {
        ...
        temp=${interface_array[$j]};
        interface_array[$j]=${interface_array[$k]};
        interface_array[$k]=$temp;
        ...
        for ((j = 0; j < $N-$i-1; j++ )); do
            ...
            case $arrayFlag in
                abc)
                    ...
                    string1=${interface_array[$j]};
                    string2=${interface_array[$j+1]};
...
arrayFlag="abc";   #string que representa qual dos arrays deve ser avaliado para determinar a ordem do sort; por padrão é por ordem alfabética
reverseFlag=0; #0 é falso, 1 é verdadeiro
loopFlag=0; #ditto ^

index=0;    #posição da opção sendo iterada; Usada para ter acesso à opção seguinte (-c)
expressionIndex=0;    #posição da opção que contem a expressão de seleção de interface
interfaceExpression=""; #string que guarda a expressão normal para seleção de interface
...
for option in "$@"; do
    ...
    if [[ $option == "-k" ]]; then
        #byteFactor=$(bc <<<"scale=3;1000/1024");
        byteFactor=1000;
        continue;
    fi
    ...
    if [[ $option == "-v" ]]; then
        #Inverte o valor lógico da flag, caso o usuário coloque -v mais de uma vez.
        let "reverseFlag -= 1"
...
while [ $loopLatch -eq 1 ]; do
    ...
    IFS=$'\n' read -r -d '' -a interface_array < <( ifconfig -a | grep ": " | awk '{print $1}' | tr -d : && printf '\0' )
    N=${#interface_array[@]};   #tamanho do array
    ...
    #Ordenar o array
    SortArrays "$arrayFlag" "$reverseFlag" "$N"
    ...
    if [ $loopFlag -eq 0 ]; then
        ...
        for (( i=0; i < $N; i++ )); do
            printf "%10s %10s %10s %10s %10s\n" ${interface_array[$i]} ${TX_array[$i]} ${RX_array[$i]} ${TRATE_array[$i]} ${RRATE_array[$i]};
        done
    #Faz o print na formatação COM a flag loop
    else
        printf "%10s %10s %10s %10s %10s %10s %10s\n" "NETIF" "TX" "RX" "TRATE" "RRATE" "TXTOT" "RXTOT";
        for (( i=0; i < $N; i++ )); do
            printf "%10s %10s %10s %10s %10s %10s %10s\n" ${interface_array[$i]} ${TX_array[$i]} ${RX_array[$i]} ${TRATE_array[$i]} ${RRATE_array[$i]} ${TXTOT_array[$i]} ${RXTOT_array[$i]};
        done
    fi
